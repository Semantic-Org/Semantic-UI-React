...
Semantic UI React provides the JavaScript for your components. You'll also need
...
<Header as='h2' content='Installation Options'/>
  <ul>
    <li><a href="#method-1-cdn">Method 1 - CDN</a></li>
    <li><a href="#method-2-semantic-ui-css-package">Method 2 - Semantic-UI-CSS</a></li>
    <li><a href="#method-3-semantic-ui-bundler-package">Method 3 - Semantic-UI Bundler Package</a></li>
...
<Message warning>
  ...
  <p>
    The release 2.3 of Semantic UI introduced some backward incompatible changes, so you should use
    corresponding version of Semantic UI React:
  </p>
  <Message.List>
...
The Semantic UI React package can be installed via Yarn (this provides the JavaScript for your components):
...
<CodeSnippet
  mode='html'
  value={`<link rel="stylesheet" href="//cdn.jsdelivr.net/npm/semantic-ui@${
    props.versions.sui
  }/dist/semantic.min.css"></link>`}
/>

## Method 2: Semantic UI CSS package
...
UI. You won't be able to use custom themes with this method.

The Semantic UI React package can be installed via Yarn (this provides the JavaScript for your components):

<CodeSnippet mode='sh' value={`$ yarn add ${props.pkg.name}`} />

Semantic UI CSS can be installed as a package in your project using Yarn (this provides the CSS package for your components).
...
## Method 3: Semantic UI Bundler Package

The full <a href={semanticUIRepoURL}>Semantic UI package</a> includes Gulp build tools so your project can preserve its own theme changes, allowing you to customise the style variables.
...
<Message warning>
  <p>
    Webpack 2/3 tree shaking does not completely remove unused exports, there are numerous issues
    that are long-standing bugs:
  </p>
  <List>
    <List.Item
      icon='github'
