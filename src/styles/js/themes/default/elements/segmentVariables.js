module.exports = (siteVars) => {
  ...
  *******************************/
  ...
  vars.stackedPageBackground = `${siteVars.subtleTransparentBlack}`
  vars.stackedPadding = `${vars.verticalPadding} + (0.4em)`
  vars.tallStackedPadding = `${vars.verticalPadding} + (0.8em)`

  /* Raised */
  vars.raisedBoxShadow = `${siteVars.floatingShadow}`
  ...
  /* Ordinality */
  vars.secondaryBackground = `${siteVars.darkWhite}`
  vars.secondaryColor = `${siteVars.mutedTextColor}`

  vars.tertiaryBackground = `${siteVars.midWhite}`
  vars.tertiaryColor = `${siteVars.mutedTextColor}`

  vars.secondaryInvertedLightness = `0.2`
  vars.secondaryInvertedBackground = `lighten(${siteVars.black}, (${vars.secondaryInvertedLightness} * 100))
    linear-gradient(
      rgba(255, 255, 255, ${vars.secondaryInvertedLightness}) 0%,
      rgba(255, 255, 255, ${vars.secondaryInvertedLightness}) 100%
    )`
  vars.secondaryInvertedColor = `${siteVars.invertedMutedTextColor}`

  vars.tertiaryInvertedLightness = `0.35`
  vars.tertiaryInvertedBackground = `lighten(${siteVars.black}, (${vars.tertiaryInvertedLightness} * 100))
    linear-gradient(
      rgba(255, 255, 255, ${vars.tertiaryInvertedLightness}) 0%,
      rgba(255, 255, 255, ${vars.tertiaryInvertedLightness}) 100%
    )`
  vars.tertiaryInvertedColor = `${siteVars.invertedMutedTextColor}`
